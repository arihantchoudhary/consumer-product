...
export default function SashaPage() {
  const AGENT_ID = "agent_0301k2s6r2pxfa4rz2dp0q99fs9d";
  
  const [showWelcome, setShowWelcome] = useState(true);
  const [showOldWelcome, setShowOldWelcome] = useState(false);
  ...
  // Check if first time visitor for old welcome modal
  useEffect(() => {
    const hasVisited = localStorage.getItem('hasVisitedSashaAssistant');
    if (!hasVisited) {
      setShowOldWelcome(true);
      localStorage.setItem('hasVisitedSashaAssistant', 'true');
    }
  }, []);
  ...
  const conversation = useConversation({
    ...
    onDebug: (debugInfo) => {
      console.log("Debug info from Sasha:", debugInfo);
    },
  });
  ...
  if (showWelcome) {
    return (
      <AccessGuard requiredPage="sasha">
        <AnimatedBackground />
  ...
  return (
    <AccessGuard requiredPage="sasha">
      ...
      {showOldWelcome && (
        <WelcomeModal onClose={() => setShowOldWelcome(false)} personName="Sasha" />
      )}
