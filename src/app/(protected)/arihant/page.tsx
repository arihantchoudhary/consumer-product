...
export default function ArihantPage() {
  ...
  useEffect(() => {
    const hasVisited = localStorage.getItem('hasVisitedArihantAssistant');
    if (!hasVisited) {
      setShowWelcome(true);
      localStorage.setItem('hasVisitedArihantAssistant', 'true');
    }
  }, []);
  ...
  // This is Arihant's page
  const pageName = "Arihant";
  ...
  return (
    <AccessGuard requiredPage="arihant">
      <AnimatedBackground />
      
      {showWelcome && (
        <WelcomeModal onClose={() => setShowWelcome(false)} personName="Arihant" />
      )}
