...
export default function LegalAssistantPage() {
  ...
  // This is Guy Ruttenberg's page
  const pageName = "Guy Ruttenberg";
  ...
  const scrollToDocumentSection = () => {
    document.getElementById('document-section')?.scrollIntoView({ 
      behavior: 'smooth' 
    });
  };
  ...
  return (
    <AccessGuard requiredPage="guy">
        ...
        <div className="max-w-4xl w-full">
          <div className="bg-black rounded-lg border border-black shadow-2xl p-12 text-center">
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-8 text-white leading-tight">
              Document Processing with {pageName}
            </h1>
            <p className="text-lg text-gray-300 mb-8 max-w-3xl mx-auto">
              Transform voice into professionally formatted legal documents using preferred templates and writing style.
            </p>
      ...
      <section id="document-section" className="relative z-10 min-h-screen flex items-center justify-center p-8 overflow-hidden">
        ...
        >
          <div className="bg-black rounded-lg border border-black shadow-2xl p-8 h-full flex flex-col justify-center">
            ...
            <p className="text-center text-gray-300 text-sm mb-6">
              Select document type to begin voice-to-draft processing
            </p>
            <div className="grid grid-cols-1 gap-4 max-w-md mx-auto">
              <Button
                onClick={() => setShowMicPanel(true)}
                variant="ghost"
                className="h-16 text-white border border-white/20 hover:bg-white/10 hover:text-white justify-start px-4"
              >
                <div className="text-left w-full">
                  <div className="font-semibold text-white">Legal Brief</div>
                  <div className="text-xs text-gray-400">Motions, pleadings, legal arguments</div>
                </div>
              </Button>
              <Button
                onClick={() => setShowMicPanel(true)}
              ...
              >
                <div className="text-left w-full">
                  <div className="font-semibold text-white">Client Letter</div>
                  <div className="text-xs text-gray-400">Correspondence, updates, advice</div>
                </div>
              </Button>
              <Button
                onClick={() => setShowMicPanel(true)}
              ...
              >
                <div className="text-left w-full">
                  <div className="font-semibold text-white">Contract Draft</div>
                  <div className="text-xs text-gray-400">Agreements, terms, clauses</div>
                </div>
              </Button>
              <Button
                onClick={() => setShowMicPanel(true)}
              ...
              >
                <div className="text-left w-full">
                  <div className="font-semibold text-white">Research Memo</div>
                  <div className="text-xs text-gray-400">Case analysis, legal research</div>
                </div>
              </Button>
            </div>
          </div>
            ...
            <div className="flex-1 flex flex-col justify-center items-center text-center space-y-8">
              <div className="space-y-2">
                <h2 className="text-xl font-semibold text-white">
                  Voice-to-Document Processing
                </h2>
                <p className="text-sm text-gray-300">
                  AI will format your dictation using your preferred templates and writing style
                </p>
              </div>
              ...
              <div className="relative flex items-center justify-center" style={{ minHeight: '160px' }}>
                ...
                <Button
                  ...
                  className={`relative w-24 h-24 rounded-full shadow-lg transition-all duration-300 hover:scale-105 z-10 ${
                    effectType === 'bounce' ? 'animate-bounce' : ''
                  } ${
                    isConversationActive ? 'ring-4 ring-green-400 ring-opacity-75 bg-green-600 hover:bg-green-700' : 'bg-blue-600 hover:bg-blue-700'
                  } ${
                    conversation.status === "connecting" ? 'ring-4 ring-yellow-400 ring-opacity-75 animate-pulse bg-yellow-600' : ''
                  }`}
                >
              ...
              <div className="text-gray-300 text-sm max-w-48 text-center">
                {conversation.status === "connecting" ? (
                  <p>Connecting to document processor...</p>
                ) : isConversationActive ? (
                  <div>
                    <p className="text-green-400 font-medium">Recording dictation for processing</p>
                    <p className="text-gray-400 text-xs mt-1">Click to end and generate document</p>
                  </div>
                ) : (
                  <p>Click to start dictating your document</p>
                )}
              </div>
            </div>
          </div>
