...
export default function SajjadPage() {
  const AGENT_ID = "agent_8001k2e22nyrem5tneenf8hqs2pf";
  ...
  useEffect(() => {
    const hasVisited = localStorage.getItem('hasVisitedSajjadAssistant');
    if (!hasVisited) {
      setShowWelcome(true);
      localStorage.setItem('hasVisitedSajjadAssistant', 'true');
    }
  }, []);
  ...
  const conversation = useConversation({
    ...
    onDebug: (debugInfo) => {
      console.log("Debug info from Sajjad:", debugInfo);
    },
  });
  ...
  return (
    <AccessGuard requiredPage="sajjad">
      <AnimatedBackground />
      
      {showWelcome && (
        <WelcomeModal onClose={() => setShowWelcome(false)} personName="Sajjad" />
      )}
